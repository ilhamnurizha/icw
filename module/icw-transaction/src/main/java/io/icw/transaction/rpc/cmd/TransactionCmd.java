...
import io.icw.transaction.model.bo.Match;
import io.icw.transaction.model.bo.Order;
import io.icw.transaction.model.bo.Trade;
import io.icw.transaction.model.bo.TxPackage;
...
//                LOG.debug("getConfirmedTransaction success. txHash:{}", txHash);
    ...
    @CmdAnnotation(cmd = "tx_trade", version = 1.0, description = "trade")
    @Parameters(value = {
            @Parameter(parameterName = "from", requestType = @TypeDescriptor(value = int.class), parameterDes = "链id"),
            @Parameter(parameterName = "to", parameterType = "String", parameterDes = "交易序列化数据字符串"),
            @Parameter(parameterName = "value", parameterType = "String", parameterDes = "交易序列化数据字符串"),
            @Parameter(parameterName = "match", parameterType = "String", parameterDes = "交易序列化数据字符串")
    })
    @ResponseData(name = "返回值", description = "返回一个Map", responseType = @TypeDescriptor(value = Map.class, mapKeys = {
            @Key(name = "value", valueType = boolean.class, description = "是否成功"),
            @Key(name = "params", description = "params")
    }))
    public Response trade(Map params) {
    	LOG.info("TransactionCmd: " + params.toString());
    	Chain chain = chainManager.getChain(1);
    	chain.getLogger().info("TransactionCmd: " + params.toString());
    	
    	Match match = JSONUtils.map2pojo(params, Match.class);
    	List<Trade> trades = match.getTrades();
    	
    	for (Trade trade : trades) {
    		Order buy = trade.getBuy();
    		Order sell = trade.getSell();
    		Transaction tx = new Transaction(TxType.COIN_BASE);
            CoinData coinData = new CoinData();
            CoinFrom coinFrom = new CoinFrom(AddressTool.getAddress(buy.getAddress()), 1, 1);
            CoinTo coinTo = new CoinTo(AddressTool.getAddress(sell.getAddress()), 1, 1, trade.getOtherAmount());
