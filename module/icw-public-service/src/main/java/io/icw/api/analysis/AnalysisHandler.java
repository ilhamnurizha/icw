package io.icw.api.analysis;

import com.fasterxml.jackson.core.JsonProcessingException;
import io.icw.api.constant.ApiConstant;
import io.icw.api.constant.CommandConstant;
import io.icw.api.model.entity.v4.TxAsset;
import io.icw.api.model.entity.v4.TxChain;
...
import io.icw.api.model.entity.*;
...
public class AnalysisHandler {
    ...
    public static ContractInfo toContractInfo(int chainId, Transaction tx, ContractResultInfo resultInfo) throws NulsException {
        ...
        boolean isNrc721 = contractInfo.getTokenType() == ApiConstant.TOKEN_TYPE_NRC721;
        if (isNrc721) {
            Object tokenName = map.get("nrc20TokenName");
            tokenName = tokenName == null ? ApiConstant.EMPTY_STRING : tokenName;
            Object tokenSymbol = map.get("nrc20TokenSymbol");
            tokenSymbol = tokenSymbol == null ? ApiConstant.EMPTY_STRING : tokenSymbol;
    ...
    public static ContractCallInfo toContractCallInfoForCrossChain(int chainId, Transaction tx) throws NulsException {
        ...
        callInfo.setGasLimit(ApiConstant.CROSS_CHAIN_GASLIMIT);
        callInfo.setPrice(ApiConstant.CONTRACT_MINIMUM_PRICE);
        callInfo.setMethodName(ApiConstant.CROSS_CHAIN_SYSTEM_CONTRACT_TRANSFER_IN_METHOD_NAME);
    ...
    private static ChainInfo toChainInfo(Transaction tx, int version) throws NulsException {
        ...
        if (version < 4) {
            ...
            io.icw.api.model.entity.v5.TxChain txChain = new io.icw.api.model.entity.v5.TxChain();
            txChain.parse(tx.getTxData(), 0);
            chainInfo.setChainId(txChain.getDefaultAsset().getChainId());
            chainInfo.setChainName(txChain.getName());

            AssetInfo assetInfo = new AssetInfo();
            io.icw.api.model.entity.v5.TxAsset txAsset = txChain.getDefaultAsset();
            assetInfo.setAssetId(txAsset.getAssetId());
            assetInfo.setChainId(txAsset.getChainId());
            assetInfo.setSymbol(txAsset.getSymbol());
            assetInfo.setInitCoins(txAsset.getInitNumber());
