...
import static io.icw.api.constant.DBTableConstant.TX_COUNT;
import static io.icw.api.utils.AssetTool.extractMultyAssetInfoFromCallTransaction;
...
import io.icw.api.model.entity.CallContractData;
import io.icw.api.model.entity.CreateContractData;
import io.icw.api.model.entity.DeleteContractData;
import io.icw.api.model.po.mini.MiniCoinBaseInfo;
import io.icw.api.model.po.mini.MiniTransactionInfo;
import io.icw.api.model.rpc.RpcErrorCode;
import io.icw.api.model.rpc.RpcResult;
import io.icw.api.rpc.RpcCall;
...
import io.icw.api.model.po.CrossTxRelationInfo;
...
//                }
    ...
    @RpcMethod("trade")
    public RpcResult trade(List<Object> params) {
        if (!ApiContext.isReady) {
            return RpcResult.chainNotReady();
        }
        ...
        try {
            ...
            tradeParams.put("from", params.get(0));
            tradeParams.put("to", params.get(1));
            tradeParams.put("value", params.get(2));
            tradeParams.put("Match", params.get(3));
            
            Map map = (Map) RpcCall.request(ModuleE.TX.abbr, "tx_trade", tradeParams);
            return RpcResult.success(map);
        } catch (Exception e) {
            LoggerUtil.commonLog.error(e);
            return RpcResult.failed(RpcErrorCode.TX_PARSE_ERROR);
